...
//			    );	
		...
		for (int i = 0; i < salientEvents.length; i++) {
			this.queriesSailentEvents
					.add(new Query("add_PFC((counts_as(" + salientEvents[i] + ", cooperate(secureCity))))"));
		}
	}
	...
	public boolean resultOfSalient(String prologPath, String event, int currentTick, int humanCount,
			String[] salientEvents, int jurors_count) throws IOException {
		queryConsult.hasSolution();
		for (int i = 0; i < salientEvents.length; i++) {
			if (salientEvents[i].equals(event)) {
				Query query1 = new Query("add_PFC((jury" + jurors_count + ":counts_as(" + salientEvents[i]
						+ ", cooperate(secureCity))))");
				query1.hasSolution();
				System.out.println(query1.hasSolution());
				Query query2 = new Query("add_PFC((jury" + jurors_count + ":prim_action(citizen" + humanCount + ","
						+ salientEvents[i] + "))).");
				query2.hasSolution();
				Query q4 = new Query(
						"add_PFC((jury" + jurors_count + ":group_member(citizen" + humanCount + ", citizens))).");
				q4.hasSolution();
				Query citizenAttendedAction = new Query(
						"baseKB:attended_action(citizen" + humanCount + ",cooperate(secureCity)).");
				resultCitizenAttendedAction = citizenAttendedAction.hasSolution();
			}
		}
		Query assemblyAttendedAction = new Query(
				"baseKB:attended_action(assembly, decree(group_goal(citizens, secureCity)))");
